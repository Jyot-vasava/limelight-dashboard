...
//       desc: `Machine was IDLE for ${Math.round(
//         longIdle.durationSec / 60
//       )} minutes`,
//       severity: "warning",
//     });
//   }
...
//       desc: `Imbalance >15% for ${Math.round(
//         bad.durationSec / 60
//       )} min — check connections!`,
//       severity: "error",
//     });
//   }
...
export const detectInsights = (points) => {
  // DEMO MODE: Force all 3 insights to appear instantly (remove after video!)
  if (points.length > 60) {
    return [
      {
        type: "idle",
  ...
  // REAL LOGIC BELOW — only runs if not in demo mode
  const insights = [];
